...
const getLocalUserByFbHandler = (request, reply) => {
  ...
  UserModel.findOne({ 'connections.facebook': request.pre.providerUser.id }, (error, data) => {
    if (error) {
      reply(Boom.serverUnavailable('Failed to get data', error))
    } else {
      if (data === null) {
        reply(data)
      } else {
        return reply({
          ...
          data,
          token: AuthUtils.createJwt(data)
        }).code(201)
      }
    }
  })
...
const getLocalUserByEmailHandler = (request, reply) => {
  if (request.pre.providerUser.email !== undefined) {
    // Fetch all data from mongodb User Collection
    UserModel.findOne({
      'email': request.pre.providerUser.email,
      'username': `${request.pre.providerUser.first_name} ${request.pre.providerUser.last_name}`
    }, (error, data) => {
      if (error) {
        // return reply(Boom.serverUnavailable('Failed to get data', error))
        console.log(error)
        reply(Boom.notFound('There is already a Facebook account that belongs to you. Sign in with that account or delete it, then link it with your current account.', data))
      } else {
        if (data) {
          reply(data)
        } else {
